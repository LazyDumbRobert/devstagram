...
@section('contenido')
    <div class="flex justify-center">
        <div class="w-full md:8/12 lg:w-6/12 flex flex-col items-center md:flex-row">
            ...
            <div class="md:w-8/12 lg:w-12 px-5 flex flex-col md:justify-center items-center md:items-start py-10 md:py-10">
               ...
               <div class="flex items-center gap-2">
                    ...
                    @auth
                        @if ($user->id === auth()->user()->id)
                            <a class="text-gray-500 hover:text-gray-500 cursor-pointer" href="{{ route('perfil.index') }}">
                ...
                <p class="text-gray-800 mb-3 text-sm font-bold" >
                    <span class="font-normal flex gap-1"> <span class="font-bold">{{ $user->followers->count() }}</span> @choice('Seguidor|Seguidores', $user->followers->count()) </span>
                </p>

                <p class="text-gray-800 mb-3 text-sm font-bold">
                    <span class="font-normal flex gap-1"> <span class="font-bold">{{ $user->followings->count() }}</span> Siguiendo </span>
                </p>

                <p class="text-gray-800 mb-3 text-sm font-bold">

                    <span class="font-normal flex gap-1"><span class="font-bold">{{ $user->posts->count() }}</span> Publicaciónes</span>
                </p>
